...
function SubmissionResultModal({ isOpen, onClose, result }: SubmissionResultModalProps) {
  useEffect(() => {
    if (!isOpen) return;
  ...
  const scoreColor =
    percentage >= 80 ? "text-green-400" : percentage >= 50 ? "text-orange-400" : "text-red-400";
  const ringColor =
    percentage >= 80 ? "stroke-green-400" : percentage >= 50 ? "stroke-orange-400" : "stroke-red-400";
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col items-center py-6">
          <div className="relative w-28 h-28">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className={`text-2xl font-bold ${scoreColor}`}>{result.totalScore}</span>
        ...
        {(result.codeQualityScore !== undefined || result.correctnessScore !== undefined) && (
          <div className="grid grid-cols-2 gap-3 px-6 pb-4">
            {result.codeQualityScore !== undefined && (
              <div className="bg-zinc-800 rounded-lg px-4 py-3 text-center">
                ...
                <p className="text-lg font-semibold text-white">{result.codeQualityScore}/40</p>
              </div>
            )}
            {result.correctnessScore !== undefined && (
              <div className="bg-zinc-800 rounded-lg px-4 py-3 text-center">
                <p className="text-xs text-zinc-500 mb-1">Correctness</p>
                <p className="text-lg font-semibold text-white">{result.correctnessScore}/60</p>
              </div>
            )}
          </div>
        )}
        ...
        <div className="px-6 pb-6">
          <p className="text-xs uppercase tracking-wide text-zinc-500 mb-2">Feedback</p>
