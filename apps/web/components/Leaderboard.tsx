...
const warriors = [
  { rank: 1, name: 'KiraHashimoto', points: 18420, solved: 412, streak: 28, change: '+0' },
  { rank: 2, name: 'voidRunner', points: 17880, solved: 398, streak: 21, change: '+1' },
  { rank: 3, name: 'quantumByte', points: 17210, solved: 376, streak: 14, change: '-1' },
  { rank: 4, name: 'neoSamurai', points: 16540, solved: 351, streak: 9, change: '+2' },
  { rank: 5, name: 'binaryPhoenix', points: 15920, solved: 340, streak: 12, change: '0' },
  { rank: 6, name: 'cipherWolf', points: 15110, solved: 322, streak: 7, change: '-2' },
];

const medal = (rank: number) => {
  if (rank === 1) return { icon: Crown, cls: 'text-ember-400' };
        ...
        <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            ...
            <p className="mt-3 max-w-xl text-slate-400">
              Ranking points are earned by solving challenges and placing in contests. Season
                ...
                >
                  <div className="col-span-2 flex items-center sm:col-span-1">
                    {m ? (
                      ...
                      <span className="font-display text-sm font-semibold text-slate-500">{w.rank}</span>
                    )}
                  </div>
                  <div className="col-span-10 sm:col-span-5">
                    <div className="flex items-center gap-3">
                      <span className="grid h-9 w-9 place-items-center rounded-lg bg-gradient-to-br from-ink-700 to-ink-800 font-display text-sm font-bold text-ember-300">
                  ...
                  <div className="col-span-2 hidden text-right text-slate-400 sm:block">{w.solved}</div>
                  ...
                  <div
                    className={`col-span-1 hidden text-right text-sm font-medium sm:block ${
                      w.change.startsWith('+') ? 'text-blade-400' : w.change.startsWith('-') ? 'text-rose-400' : 'text-slate-500'
                    }`}
                  >
                    {w.change}
                  </div>
